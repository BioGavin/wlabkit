...
os.chdir('/mnt/d/desktop/smiles')  #test.tsv路径
path=os.getcwd()
filename='test'
def get_molecualr_similarity(path,filename,threshold):
    file='/{}/{}.tsv'.format(path,filename)
    ...
    node2_type0,node2_type1,node2_type2=[],[],[]
    for i in tqdm(a.index):
        for j in range(i):
            ...
            if tanimoto >= threshold:
                ...
                node1_type1.append(a.CLUSTER[i])
                node2_type1.append(a.CLUSTER[j])
                node1_id.append(i)
                node2_id.append(j)
...
if __name__ == '__main__':
    for i in [0.6]:
        get_molecualr_similarity(path,filename,i)
